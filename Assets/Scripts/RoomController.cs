...
public class RoomController : MonoBehaviour
{
    ...
    void Start()
    {
        isPlayer = ((SpaceshipController)transform.parent.GetComponent<MonoBehaviour>())
            .spaceship
            .isPlayer;
        Setup(roomType);
    }
    ...
    void OnMouseDown()
    {
        ...
        {
            GameManagerController.Instance.RoomClicked(
    ...
    Room createRoom(RoomType roomType, bool getEveryRoom = false)
    {
        ...
        {
            return new Room(laserActions, roomType, 5);
        }
        else if (roomType == RoomType.Shield)
        {
            return new Room(shieldActions, roomType, 3);
        }
        else if (roomType == RoomType.Engine)
        {
            return new Room(engineActions, roomType, 5);
        }
        else if (roomType == RoomType.Reactor)
        {
            return new Room(reactorActions, roomType, 8);
        }
        else if (roomType == RoomType.Missile)
        {
            return new Room(missileActions, roomType, 3);
        }
        else if (roomType == RoomType.Firebomb)
        {
            return new Room(firebombActions, roomType, 3);
        }
        else if (roomType == RoomType.ShieldPiercer)
        {
            return new Room(shieldPiercerActions, roomType, 3);
        }
        else if (roomType == RoomType.EvasiveManeouvre)
        {
            return new Room(evasiveManeouvreActions, roomType, 3);
        }
        else if (roomType == RoomType.Battery)
        {
            return new Room(batteryActions, roomType, 3);
        }
        else if (roomType == RoomType.EMP)
        {
            return new Room(EMPActions, roomType, 3);
        }
        else
            return new Room(new List<CardAction>(), RoomType.Reactor, 0);
    }
}
...
public class Room
{
    ...
    public Spaceship getParentShip(bool invert = false)
    {
        ...
        {
            if (parent.isPlayer != invert)
            {
                return GameManagerController.Instance.playerShip;
            }
        ...
        else
        {
            if (parent.isPlayer != invert)
            {
                return actions[0].state.playerShip;
            }
    ...
    public void IncreaseAttackIntent(float damage)
    {
        ...
        UpdateTextGraphics();
        parent.shieldBarRenderer.material.SetFloat(
            "_FlashingSegments",
            Math.Min(this.incomingDamage, this.defence)
        );
        parent.healthBarRenderer.material.SetFloat(
            "_FlashingSegments",
            Math.Max(this.incomingDamage - this.defence, 0)
        );
    }
    ...
    public void UpdateTextGraphics()
    {
        ...
        Health.text = getHealth() + "/" + getMaxHealth();
        Attack.text = (incomingDamage - defence).ToString();
        ...
        parent.shieldBarRenderer.material.SetFloat(
            "_RemovedSegments",
            this.getMaxHealth() - this.defence
        );
    }
