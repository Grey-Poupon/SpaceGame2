...
public abstract class CombatEffect
{
    ...
    public CombatEffect Clone()
    {
        ...
        foreach (
            FieldInfo sourceField in sourceType.GetFields(
...
public class GeneralShieldEffect : CombatEffect
{
    ...
    public override void TriggerEffect()
    {
        ...
        else
        {
            ...
            {
                foreach (Room room in action.state.playerShip.rooms.Values.SelectMany(x => x).ToList())
                {
                    room.IncreaseDefence(increase);
                }
            }
            ...
            {
                foreach (Room room in action.state.enemyShip.rooms.Values.SelectMany(x => x).ToList())
                {
                    room.IncreaseDefence(increase);
                }
            }
...
public class FreeLaserEffect : CombatEffect
{
    ...
    public override void TriggerEffect()
    {
        ...
        {
            List<Card> cards = GameManagerController.Instance.MakeCards(
                new List<CardAction> { freeAction },
                affectsPlayer
            );
...
public class OnFireEffect : CombatEffect
{
    ...
    public override void FirstEffect()
    {
        ...
        if (duration == startDuration)
        {
            ...
            {
                List<Card> cards = this.action.state.MakeCards(
                    new List<CardAction> { action },
                    false
                );
                this.action.state.AddCardsToHand(cards, affectsPlayer);
...
public class StopFireEffect : CombatEffect
{
    ...
    public override void TriggerEffect()
    {
        OnFireEffect onFireEffect =
            affectedRoom.effectsApplied.FirstOrDefault(item => item is OnFireEffect)
            as OnFireEffect;
...
public class APEffect : CombatEffect
{
    ...
    public override void TriggerEffect()
    {
        if (action.state == null)
        {
            if (affectsSelf == action.sourceRoom.isPlayer)
            {
                GameManagerController.Instance.playerShip.AdjustAP(change);
            }
            else
            {
                GameManagerController.Instance.enemyShip.AdjustAP(change);
            }
        }
        else
        {
            if (affectsSelf == action.sourceRoom.isPlayer)
            {
                action.state.playerShip.AdjustAP(change, true);
            }
            else
            {
                action.state.enemyShip.AdjustAP(change, true);
            }
        }
    }
...
public class SpeedEffect : CombatEffect
{
    ...
    public override void TriggerEffect()
    {
        if (action.state == null)
        {
            if (affectsSelf == action.sourceRoom.isPlayer)
            {
                GameManagerController.Instance.playerShip.AdjustSpeed(change);
            }
            else
            {
                GameManagerController.Instance.enemyShip.AdjustSpeed(change);
            }
        }
        else
        {
            if (affectsSelf == action.sourceRoom.isPlayer)
            {
                action.state.playerShip.AdjustSpeed(change, true);
            }
            else
            {
                action.state.enemyShip.AdjustSpeed(change, true);
            }
        }
    }
...
public class ChargeBatteriesEffect : CombatEffect
{
    ...
    public override void TriggerEffect()
    {
        ...
        if (this.action.state == null)
        {
            List<Card> cards = GameManagerController.Instance.MakeCards(
                new List<CardAction> { action },
                affectsPlayer && this.action.state == null
            );
            GameManagerController.Instance.AddCardsToHand(cards, affectsPlayer);
        }
        else
